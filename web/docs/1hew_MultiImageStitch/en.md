...
**Node Purpose:** `Multi Image Stitch` stitches multiple images in a chosen direction (`top`, `bottom`, `left`, `right`) with configurable spacing width and color. Supports match-by-resize or match-by-padding and handles batches safely. If any input has alpha, all processing switches to RGBA and the output automatically keeps alpha.
...
- Channel rule: if any input has alpha, images, spacer strips, and padded areas are all handled in RGBA and the output preserves alpha automatically.
- Two matching modes:
