...
**节点功能：** `Image Blend Mode by CSS` 节点以接近 CSS 的混合语义在基础图层与叠加图层之间进行融合，支持 `blend_mode`、整体 `blend_percentage` 不透明度与可选的 `overlay_mask` 控制，含 HSL 模式：`hue`、`saturation`、`color`、`luminosity`，并支持 RGBA 输入。
...
- 若希望最终输出保留透明通道，请让 `base_image` 使用 RGBA 输入。
