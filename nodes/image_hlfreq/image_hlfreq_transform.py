...
class ImageHLFreqTransform(io.ComfyNode):
    """高级细节迁移节点 - 支持RGB/HSV/IGBI三种频率分离方法"""
    @classmethod
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="1hew_ImageHLFreqTransform",
            display_name="Image HL Freq Transform",
            category="1hewNodes/image/hlfreq",
            inputs=[
                io.Image.Input("generate_image"),
                io.Image.Input("detail_image"),
                io.Mask.Input("detail_mask", optional=True),
                io.Combo.Input("method", options=["rgb", "hsv", "igbi"], default="igbi"),
    ...
    async def execute(
        cls,
        generate_image: torch.Tensor,
        detail_image: torch.Tensor,
        method: str,
        blur_radius: float,
        ...
        for i in range(batch_size):
            ...
            if method == "igbi":
                result, high_freq, low_freq = cls._igbi_detail_transfer(
                    generated_img, detail_img, current_mask, effective_blur_radius
                )
            else:
                result, high_freq, low_freq = cls._traditional_detail_transfer(
    ...
    @classmethod
    def _igbi_detail_transfer(cls, generate_img, detail_img, mask, blur_radius):
        """
        IGBI模式细节迁移 - 修正版本
        低频图片：生成图像高斯模糊后的结果
        ...
        # 确保blur_radius为奇数
        blur_radius = cls._ensure_odd_radius(blur_radius)
        ...
        # 应用色阶调整得到最终结果
        result_uint8 = cls._apply_levels_numpy(result_mixed, 83, 172, 1.0, 0, 255)
    ...
    @classmethod
    def _traditional_detail_transfer(cls, generated_img, detail_img, mask, method, base_radius):
        blur_radius = cls._ensure_odd_radius(base_radius)
        generated_blur_radius = cls._ensure_odd_radius(base_radius * 1.5)
        
        # 单次处理
        detail_high_freq, _ = cls._single_separation(detail_img, method, blur_radius)
        generated_high_freq, generated_low_freq = cls._single_separation(
        ...
        # 重组图像
        if method == "hsv":
            result = cls._recombine_hsv(mixed_high_freq, generated_low_freq)
        else:
            result = cls._recombine_linear_light(mixed_high_freq, generated_low_freq)
        result = np.clip(result, 0, 1)
        
        return result, mixed_high_freq, generated_low_freq
    
    @staticmethod
    def _recombine_hsv(high_freq, low_freq):
    ...
    @classmethod
    def _single_separation(cls, image, method, blur_radius):
        if method == "rgb":
            return cls._rgb_separation(image, blur_radius)
        else:
            return cls._hsv_separation(image, blur_radius)
    
    @staticmethod
    def _rgb_separation(image, blur_radius):
        blur_radius = ImageHLFreqTransform._ensure_odd_radius(blur_radius)
    ...
    @staticmethod
    def _hsv_separation(image, blur_radius):
        blur_radius = ImageHLFreqTransform._ensure_odd_radius(blur_radius)
    ...
    @staticmethod
    def _recombine_linear_light(high_freq, low_freq):
        result = 2 * high_freq + low_freq - 1
        return np.clip(result, 0, 1)
    
    @staticmethod
    def _apply_levels_numpy(image_array, black_point, white_point, gray_point=1.0, output_black_point=0, output_white_point=255):
        ...
        for i in range(image_array.shape[2]):
            ...
            denom = max(1, (white_point - black_point))
            channel = (channel - black_point) / denom * 255.0
            if gray_point != 1.0:
