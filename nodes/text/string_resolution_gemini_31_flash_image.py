"""Gemini 3.1 Flash Image 专用分辨率标签节点。

与 1hewOffice 的 1hewOffice_Gemini31FlashImage 成对使用：按图像面积推断出 API 节点
resolution 支持的档位之一（0.5k / 1k / 2k / 4k），未连接图像时直接透传手动选择的标签。

目标面积取自各档 1:1 官方尺寸（1k 为 1024x1024，其余按 0.5/1/2/4 倍缩放）：
0.5k = 512x512，1k = 1024x1024，2k = 2048x2048，4k = 4096x4096。
"""
...
class StringResolutionGemini31FlashImage(io.ComfyNode):
    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="1hew_StringResolutionGemini31FlashImage",
            display_name="String Resolution Gemini 3.1 Flash Image",
            category="1hewNodes/text",
            description=(
                "从输入图像推断最接近的 Gemini 3.1 Flash Image 分辨率档位（0.5k / 1k / 2k / 4k）；未连接图像时透传所选标签。与 1hewOffice 的 Gemini 3.1 Flash Image 节点成对使用。"
            ),
            inputs=[
