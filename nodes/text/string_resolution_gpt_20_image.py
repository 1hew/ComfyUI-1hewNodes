"""GPT Image 2.0 专用分辨率标签节点。

与 1hewOffice 的 1hewOffice_GPTImage20 成对使用：按图像面积推断出 API 节点
resolution 支持的档位之一（1k / 2k / 4k），未连接图像时直接透传手动选择的标签。

目标面积取自 GPTImage20.MAX_PIXELS_BY_RESOLUTION（该文件属于 1hewOffice，不在本仓库内）：
1k = 1024x1024，2k = 2048x2048，4k = 3840x2160（与 4k 的 1:1 尺寸 2880x2880 同面积）。
"""
...
class StringResolutionGpt20Image(io.ComfyNode):
    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="1hew_StringResolutionGpt20Image",
            display_name="String Resolution GPT Image 2.0",
            category="1hewNodes/text",
            description=(
                "从输入图像推断最接近的 GPT Image 2.0 分辨率档位（1k / 2k / 4k）；未连接图像时透传所选标签。与 1hewOffice 的 GPT Image 2.0 节点成对使用。"
            ),
            inputs=[
