...
class ImageTileSplitPreset:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                "image": ("IMAGE",),
                "overlap_amount": ("FLOAT", {"default": 0.05, "min": 0.0, "max": 512.0, "step": 0.01}),
                "tile_preset_size": (preset_options, {"default": "auto"}),
            }
        }

    RETURN_TYPES = ("IMAGE", "DICT")
    RETURN_NAMES = ("tiles", "tiles_meta")
    FUNCTION = "image_tile_split_predefined"
    CATEGORY = "1hewNodes/image/tile"

    def calculate_best_tile_size(self, image_width, image_height, tile_preset_size):
        """根据用户偏好选择tile尺寸"""
        # 如果用户指定了具体尺寸
        ...
        if best_size is None:
            # 如果没有找到合适的尺寸，使用最小的尺寸
            smallest_preset = min(self.PRESET_RESOLUTIONS, key=lambda x: x[1] * x[2])
    ...
    def tile_image(self, image, tile_width, tile_height, overlap_width, overlap_height):
        """图像分割方法，使用预定义尺寸"""
        image_pil = self.tensor2pil(image.squeeze(0))
        img_width, img_height = image_pil.size
    ...
    def image_tile_split_predefined(self, image, overlap_amount, tile_preset_size):
        _, img_height, img_width, _ = image.shape
        
        # 选择最佳的预定义tile尺寸
        (tile_width, tile_height), (rows, cols), size_info = self.calculate_best_tile_size(
            img_width, img_height, tile_preset_size
        )
